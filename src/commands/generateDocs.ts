...
export class GenerateDocsCommand {
    ...
    public async execute(): Promise<void> {
        ...
        if (docFormats.length === 0) {
            vscode.window.showErrorMessage(`Documentation generation is not supported for ${languageId} files.`);
            return;
        }
        ...
        const docType = await vscode.window.showQuickPick([
            { label: 'Inline Documentation', description: 'Add documentation comments to the existing code' },
            { label: 'Separate Documentation File', description: 'Generate a separate documentation file' }
        ], {
